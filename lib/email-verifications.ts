...
export async function createEmailVerificationToken(input: { usuarioId: string; email: string }) {
...
export async function sendEmailVerification(input: { nome: string; email: string; token: string }) {
    ...
    html: `
      <div style="font-family:Arial,sans-serif;background:#0f172a;color:#e2e8f0;padding:32px;">
        <div style="max-width:560px;margin:0 auto;background:#111827;border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:32px;">
          ...
          <a href="${verificationUrl}" style="display:inline-block;background:#0ea5e9;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:14px;font-weight:700;">
            Confirmar email
          </a>
...
export async function consumeEmailVerificationToken(token: string) {
  ...
  const row = data as EmailVerificationRow | null;
  ...
  if (usuarioError) {
    return { ok: false as const, reason: "user_update_failed" };
  }
  ...
  if (verificationError) {
    return { ok: false as const, reason: "verification_update_failed" };
  }

  return { ok: true as const, usuarioId: row.usuario_id };
}
